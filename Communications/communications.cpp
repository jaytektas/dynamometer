...
{
DCommunications::DCommunications(QObject *parent) : QObject (parent)
{
}

DCommunications::~DCommunications()
{
    closeSerial();
    if (c_serialPort) delete c_serialPort;
}

bool DCommunications::openSerial()
{
    if (!c_connected)
    {
        if (!c_serialPort)
            c_serialPort = new QSerialPort(this);

        c_serialPort->setPortName(c_portName);
        c_serialPort->setBaudRate(c_baudRate);
        c_serialPort->setDataBits(c_dataBits);
        c_serialPort->setParity(c_parity);
        c_serialPort->setStopBits(c_stopBits);
        c_serialPort->setFlowControl(c_flowControl);

        if (!c_serialPort->open(QIODevice::ReadWrite))
        {
            c_connected = false;
            return false;
        }
...
void DCommunications::closeSerial()
{
    if (c_connected)
    {
        if (c_serialPort->isOpen())
            c_serialPort->close();
        delete c_serialPort;
        c_serialPort = nullptr;
        emit connected(false);
    }
    c_connected = false;
}

QByteArray DCommunications::transmit(char command, QByteArray data)
{
char c;
int seen = 0;
SerialState_t state = START;
int s = 0;
QByteArray envelope, crc;

    if (!c_connected) return nullptr;
    if (!c_serialPort) return nullptr;

    c_serialPort->write(envelopePacket(command, data));
    if (!c_serialPort->waitForBytesWritten(c_timeout))
        return nullptr;

    // collect the response or timeout
    for(;;)
    {
        if (!c_serialPort->bytesAvailable())
            if (!c_serialPort->waitForReadyRead(c_timeout))
                return nullptr;

        if (state == START)
        {
            if (!c_serialPort->getChar(&c)) continue;
        ...
        if (state == HEADER)
        {
            ...
            seen++;
            envelope += c;
            if (seen >= PACKET_HEADER)
            {
                state = PACKET;
                // get the size of packet from the header
                s = (envelope[3] << 8 | envelope[4]);
            }
        }

        if (state == PACKET)
        {
            // always try and solve by a readAll
            if (seen + c_serialPort->bytesAvailable() <= s)
            {
                seen += c_serialPort->bytesAvailable();
                envelope += c_serialPort->readAll();
            }
            else
            {
                if (c_serialPort->getChar(&c))
                {
                    seen++;
                    envelope += c;
                }
        ...
        {
            // check crc
            crc = crc32(envelope, envelope.length());
            if (crc.compare(QByteArray("\x00\x00\x00\x00")))
                return envelope;
        }
    }
}

QByteArray DCommunications::envelopePacket(char command, QByteArray packet)
{
QByteArray ep;
unsigned short l;

    l = static_cast<unsigned short>(PACKET_HEADER + PACKET_CRC + packet.length());
    ep = QByteArray("\x01\x01") + command + static_cast<char>(l >> 8) + static_cast<char>(l & 0xff) + packet;
    ep += crc32(ep, ep.length());
    return ep;
}
...
QByteArray DCommunications::crc32(QByteArray data, int len)
{
    ...
    for(i = 0; i < len; i++)
    {
        uint8_t top = (Checksum >> 24);
        top ^= data[i];
        Checksum = (Checksum << 8) ^ crc_table[top];
    }
    cs += static_cast<char>(Checksum >> 24 & 0xff);
    cs += static_cast<char>(Checksum >> 16 & 0xff);
    cs += static_cast<char>(Checksum >> 8 & 0xff);
    cs += static_cast<char>(Checksum & 0xff);
